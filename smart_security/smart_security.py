...
def replace_with_defaults(function):
    ...
    def inner(self, *args, **kwargs):
        ...
        index_of_first_kwarg_not_passed_using_position = max(len(args) + 1, len(arguments) - len(defaults))
        for key_value_argument in arguments[index_of_first_kwarg_not_passed_using_position:]:
            # Adding to kwargs defaults from self
            if key_value_argument not in kwargs and hasattr(self, default_marker + key_value_argument):
                kwargs[key_value_argument] = getattr(self, default_marker + key_value_argument)
...
class SmartSecurity(object):
    ...
    many similar decorators, and passing lot of them in every methods responsible for handling requests.
    A user should define a owner's model. Id's of elements of this model should be accessible from request variable.
    Than this library can check (for any argument of a decorated method) whether has user permission to this object.
    """

    @classmethod
    def ignore_check(cls, model_class):
        """
        This decorator is used for determine, which models should not be checked.
        @param model_class: model class for which permission check should be skipped
    ...
    def check_permissions(cls, function):
        ...
        For each argument with a name matching to pattern <model_name>_id he finds a way to owner's class
        ...
        def inner(request, *args, **kwargs):
            ...
            merged_arguments = SmartSecurity._merge_args_and_kwargs(function, (request,) + args, kwargs)
            ...
            for key, value in merged_arguments:
                ...
                if model_class is None or not SmartSecurity._is_integer(value):
                    continue
                    # If name of an argument doesn't match to any existing model than checking is continued.
    ...
    def if_user_has_no_permissions(cls, request, key, value):
        """
        This method determines what to do if user hasn't permissions to requested object.
        @param request:
    ...
    def get_all_models_candidates(cls):
        ...
        def should_not_be_checked(model):
            return not (hasattr(model, '_meta') and hasattr(
                    model._meta, 'ignore_smart_security') and model._meta.ignore_smart_security)
    ...
    def get_model_for_argument(cls, key):
        ...
        model_classes = [model for model in all_models if model.__name__ == key_as_model_name]
        ...
        if number_of_models_matching_criteria == 0:
            ...
            """.format(key, model_classes[0].__module__))
        return model_classes[0]

    @staticmethod
    def _convert_variable_to_class_name(variable_name):
        # This method converts argument names (like "some_model_id" to class names (like "SomeModel")
        import re
    ...
    @staticmethod
    def _merge_args_and_kwargs(function, positional_arguments_to_merge, key_value_arguments_to_merge):
...
def smart_security_decorator(check_permissions=None, if_user_has_no_permissions=None, get_owner_id_from_request=None,
                             get_owner_class=None, get_all_models_candidates=None, get_model_for_argument=None):
    """ A decorator which work exactly the same as classes derived from SmartSecurity.
    He requires that all arguments must be functions with class instance as first parameter.
    ...
    methods_to_override = \
        [("check_permissions", check_permissions), ("if_user_has_no_permissions", if_user_has_no_permissions),
         ("get_owner_id_from_request", get_owner_id_from_request), ("get_owner_class", get_owner_class),
         ("get_all_models_candidates", get_all_models_candidates), ("get_model_for_argument", get_model_for_argument)]
    methods_to_override = filter(lambda x: x[1] is not None, methods_to_override)
    methods_to_override_as_class_methods = map(lambda method: (method[0], classmethod(method[1])), methods_to_override)
    new_type = type('SmartSecurityAnonymousClass', (SmartSecurity,), dict(methods_to_override_as_class_methods))
    return new_type.check_permissions


class ModelOwnerPathFinder(object):
    """
    This class is responsible for finding the shortest path to the owner's model.
    An application can be treated as a graph. Models are vertexes and relationships are edges.
    """
    ...
    def find_shortest_path_to_owner_model(self, model_to_search_class=None, security_model_class=None):
        """
        ...
        def process_ancestors():
            # This method creates argument name to access owner's class from model manager of model_to_search_class
            result = ""
    ...
    @replace_with_defaults
    def detect_has_user_object_access(self, model_id, owner_id, model_to_search_class=None,
        ...
        return shortest_path_to_owner is None or model_to_search_class.objects.filter(
                **{'pk': model_id, shortest_path_to_owner: owner_id}).exists()
